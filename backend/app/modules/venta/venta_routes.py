...
def create():
    try:
        ...
        if venta:
            return jsonify({"mensaje": "Venta creado exitosamente"}), 201
        else:
...
def update(id):
    try:
        ...
        if venta:
            return jsonify({"mensaje": "Venta actualizado exitosamente"}), 200
        else:
...
def delete(id):
    try:
        ...
        if venta:
            return jsonify({"mensaje": "Venta eliminado exitosamente"}), 200
        else:
